...
function install_arch(){ 
    echo "Installing required packages"
    pacman -S --needed python3 tk git && python3 -m ensurepip unzip || install_fail
} 

# Function to install for debian (and ubuntu)
function install_deb(){ 
    echo "Installing required packages"
    apt install -y python3 python3-dev python3-tk python3-pip unzip || install_fail
} 

# Function to install for fedora (and other forks)
function install_fedora(){ 
    echo "Installing required packages" 
    dnf install python3 python3-tkinter python3-pip python3-devel unzip || install_fail
} 

# Function to install for centos (and other forks based on it)
function install_centos(){
    echo "Installing required packages"
    yum install -y python3 || install_fail
    yum install -y python3-tkinter epel-release python3-pip unzip|| install_fail
} 
